...
export const TrialTracker = ({ subscriptions, onEdit }: TrialTrackerProps) => {
  ...
  if (trialSubscriptions.length === 0) {
    return (
      <EmptyState
        icon={CheckCircle}
        title={t('subscriptions.noActiveTrialsTitle')}
        description={t('subscriptions.noActiveTrialsDesc')}
      />
    );
  }
          ...
          <Card key={sub.id} className={urgency === 'critical' ? 'border-destructive' : urgency === 'warning' ? 'border-warning' : ''}>
                ...
                <div className={`flex items-center gap-3 ${rtlClass(isRTL, 'flex-row-reverse', 'flex-row')}`}>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {t('subscriptions.trialEnds')} {format(new Date(sub.trial_ends_at), 'MMM d, yyyy')}
                    </p>
                  </div>
                </div>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between text-sm">
                <span className="text-muted-foreground">{t('subscriptions.willConvertTo')}</span>
                <span className="font-semibold">{formatCurrency(Number(sub.amount), sub.currency, i18n.language)}/{sub.billing_cycle}</span>
              </div>

              {urgency === 'critical' && (
                <div className={`flex items-start gap-2 p-3 bg-destructive/10 border border-destructive/20 rounded-lg ${rtlClass(isRTL, 'flex-row-reverse', 'flex-row')}`}>
                  <AlertTriangle className="h-4 w-4 text-destructive mt-0.5" />
                  <div className="text-sm">
                    <p className="font-medium text-destructive">{t('subscriptions.trialEndingSoon')}</p>
                    <p className="text-muted-foreground">{t('subscriptions.cancelToAvoidCharge')}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => onEdit(sub)} className="flex-1">
                  {t('subscriptions.manageTrial')}
                </Button>
                <Button variant="destructive" size="sm" className="flex-1">
                  {t('subscriptions.cancelBeforeCharge')}
                </Button>
              </div>
            </CardContent>
          </Card>
