...
export function HabitsDueTodayWidget({ habits, onRefresh }: HabitsDueTodayWidgetProps) {
  ...
  return (
    <DashboardWidgetCard 
      title={t('dashboard.habitsToday') || "Habits Today"}
      icon={CheckCircle}
      ...
      {habits.length > 0 ? (
        <div className="space-y-3">
          ...
          <div className={isRTL ? 'text-right' : 'text-left'}>
            <div className="flex items-center justify-between mb-1 text-xs text-muted-foreground">
              <span>{t('dashboard.todaysProgress') || "Today's Progress"}</span>
          ...
          {uncheckedHabits.length > 0 ? (
                ...
                >
                  ...
                  <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'}`}>
                    ...
                    <div className={`flex items-center gap-1 text-xs text-muted-foreground ${isRTL ? 'flex-row-reverse' : ''}`}>
                      ...
                      <span>{habit.streak_count} {t('habits.dayStreak') || 'day streak'}</span>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className={`text-center py-4 ${isRTL ? 'text-right' : 'text-left'}`}>
              <p className="text-sm font-medium text-success mb-1">
                {t('dashboard.allHabitsComplete') || "All habits completed! 🎉"}
              </p>
              <p className="text-xs text-muted-foreground">
                {t('dashboard.keepItUp') || 'Keep up the great work!'}
              </p>
            </div>
          )}
        ...
        <div className={`text-center py-6 ${isRTL ? 'text-right' : 'text-left'}`}>
          <p className="text-sm text-muted-foreground mb-2">
            {t('dashboard.noHabits') || "No habits yet"}
          </p>
          <Button 
            variant="outline" 
            size="sm" 
            onClick={() => navigate('/habits')}
          >
            {t('dashboard.createHabit') || 'Create Habit'}
          </Button>
        </div>
      )}
    </DashboardWidgetCard>
